...
- **🛡️ REGRA NOVA (22 ago, nasceu de erro do copiloto): CÓDIGO SÓ CHEGA AO DONO DEPOIS DE COMPILAR NA BANCATA.** Toda entrega de código passa por `tsc --noEmit` (TypeScript strict, com stubs de next/server e supabase — bancata-ts/) ANTES de ir pro dono. **NUNCA entregar rascunho/revisão.** Cicatriz que gerou a lei: v2 do route.ts entregue corrompida + v3 com erro de sintaxe (`}?;`) — custaram 3 rodadas de porteiros do dono. Tempo do dono é o bem mais caro da casa.
