...
|---|---|---|---|
| `src/app/api/imagem/route.ts` | `codigo/019_api_imagem_route.ts` | **v11** | `Pollinations` = 10 |
| `src/app/api/ia/route.ts` | `codigo/019_api_ia_route.ts` | **v2** | `anotarDetalheIA` = 5 |
...
1. `api\imagem\route.ts` → busca `Pollinations` → **10**
2. `api\ia\route.ts` → busca `anotarDetalheIA` → **5**
