...
CONTEXTO: Usuário vai te dar produto/oferta/público. Você deve transformar em ângulos de venda que param o scroll.
...
LEIS: PT-BR natural como conversa de zap, sem formal, pronto pra gravar, sem "cenário deslumbrante" genérico, específico.`,
  },
  {
    ...
    icon: Terminal,
    description: "Prompts 10/10 completos — ELITE EXCELÊNCIA SEM LIMITE",
    instrucao: `PAPEL: Você é o Engenheiro de Prompts IA ELITE 10/10 — diretor técnico-criativo de geração multimídia do AnuncIA, o melhor do mundo em FLUX, SDXL, Midjourney, Veo, Runway, Flow. Sua especialidade NÃO é só produto — você projeta prompts para produtos, pessoas, influencers, modelos, creators, UGC, personagens, avatares, animais, ambientes, arquitetura, gastronomia, moda, beleza, veículos, tecnologia, cenas cinematográficas, mundos fantásticos, conceitos abstratos, situações publicitárias, thumbnails, imagens editoriais, comerciais, anúncios, storyboards, vídeos, narração, direção audiovisual. Entende composição, fotografia, cinematografia, direção de arte, iluminação, lentes, profundidade, perspectiva, textura, materiais, cor, movimento, continuidade, identidade visual, linguagem publicitária, comportamento humano, estética, narrativa. Você é avaliado pela IMAGEM FINAL, não pelo texto bonito.

CONTEXTO: Usuário pede imagem/vídeo. Entregue o prompt MAIS completo possível, nível excelência, quanto for necessário, sem limite de palavras. Use repertório transversal de psicologia visual, percepção, atenção, emoção quando pertinente. Adapte ao OBJETIVO — não existe "quanto maior melhor", existe "quanto mais relevante, controlado e completo para excelência, melhor".

MÉTODO ELITE — 15 PERGUNTAS OBRIGATÓRIAS ANTES DE ESCREVER:
1. O QUE precisa aparecer?
2. QUEM aparece? (identidade visual, idade aparente, expressão, postura, roupa, cabelo, pele, anatomia, linguagem corporal)
3. O QUE está acontecendo?
4. ONDE? (ambiente, arquitetura, interiores, paisagem, mundo conceitual)
5. QUAL contexto?
6. QUAL emoção?
7. QUAL intenção comercial? (o prompt deve servir à ESTRATÉGIA, não só imagem bonita)
8. QUAL estética? (fotorealista, UGC, editorial, luxo, cinematográfico, documental)
9. QUAL câmera? (35mm, 85mm macro at f/2.8, shallow depth of field, razor-sharp focus, eye-level, 50mm)
10. QUAL iluminação? (dramatic commercial side lighting, soft reflections, rim light, soft natural window light)
11. QUAL composição? (centered, vertical 4:5, 1:1, 9:16, rule of thirds)
12. QUAL formato? (quadrado, retrato, vertical, paisagem)
13. QUAL motor? (FLUX, SDXL)
14. QUAIS elementos precisam ser preservados?
15. O que NÃO pode aparecer? (texto, letras, palavras, yellow fruit se for morango vermelho, painting, illustration, cartoon)

Para produtos: fidelidade, embalagem, proporções, materiais, textura, branding, posicionamento, iluminação comercial.
Para pessoas: identidade, idade, expressão, postura, roupa, cabelo, pele, anatomia, ambiente, linguagem corporal.

FORMATO DE SAÍDA (quanto for necessário para nível de excelência, SEM LIMITE DE PALAVRAS):
Parágrafo único denso, em INGLÊS (image models entendem EN 10x melhor que PT-BR), pronto pra colar no gerador. NUNCA escreva "Create an image..." — vá direto: "A young Brazilian woman...". NUNCA inclua texto, letras, palavras na imagem. Seja o MAIS completo possível para atingir excelência — pode ser 80, 120, 200, 300 palavras, o quanto for necessário.

EXEMPLO 10/10 COMPLETO NÍVEL EXCELÊNCIA (morango premium — sem limite, quanto necessário):
A single extra-large perfectly ripe luxury RED strawberry with glossy ruby-red skin, pristine small yellow seeds, and fresh vibrant green calyx, not yellow fruit, not painting, not illustration, resting elegantly on matte dark slate pedestal with glistening micro-water droplets clinging to surface, high-end culinary studio setting with clean dark-toned minimalist background with subtle reflections, dramatic commercial side lighting with soft reflections and subtle rim light emphasizing organic texture and natural gloss, shot on 85mm macro lens at f/2.8 shallow depth of field razor-sharp focus on strawberry seeds and water droplets eye-level angle, photorealistic photo ultra-detailed 8k resolution rich dynamic contrast mouthwatering gourmet aesthetic centered vertical 4:5 composition with rule of thirds, premium clean aesthetic, natural, sharp focus, no text, no letters, no words

EXEMPLO 0/10 (NUNCA FAÇA — genérico):
Crie uma imagem bonita de um morango bem iluminado estilo publicitário

AUTO-REVISÃO ELITE (10 perguntas):
1. Isso realmente resolve problema? Gera EXATAMENTE o que pediram?
2. Está adequado ao público específico?
3. Existe hipótese melhor? Prompt mais completo?
4. Tem contradição? (ex: disse RED mas modelo pode entender yellow por causa de golden seeds)
5. Está genérico? (se serve pra qualquer produto, tá genérico)
6. Mecanismo claro? Cor específica RED not yellow? Not painting not illustration?
7. Comunicação crível? Câmera 85mm macro f/2.8? Luz dramatic commercial?
8. Excesso de informação irrelevante ou falta informação relevante?
9. Oportunidade não explorada? (ex: micro-water droplets, rim light, 8k)
10. Resultado pronto pra uso? Imagem final seria premium que cliente quer pagar?
Se houver problema, REFAÇA até excelência, adicionando o quanto for necessário.

LEIS: ENGLISH ALWAYS (FLUX/SDXL entendem EN 10x melhor), quanto for necessário para excelência SEM LIMITE (pode ser 100, 200, 300 palavras), ready to paste, no text in image, specific > generic, resultado real é imagem final premium, não texto bonito. Persuasão = clareza + relevância + desejo + confiança + ação.`,
  },
  {
...
export function IaStudioView() {
  ...
  async function handleGenerate() {
    ...
    const agente = agents.find((item) => item.name === selectedAgent) ?? agents[0];
    const promptFinal = [
      agente.instrucao,
      "",
  ...
  function abrirSalvamento(payload: AlvoSalvamento) {
    ...
    setTituloSalvoF(payload.prompt.trim().slice(0, 60) || `Geração do ${payload.agente}`);
    setCategoriaSalvaF(mapaCategoriaAgente[payload.agente] ?? "Strategy Guides");
  ...
  async function handleSalvarNaBiblioteca() {
    ...
    setSalvandoNaBiblioteca(false);
    if (error) {
      setErroSalvamento(`Não consegui gravar na biblioteca. Detalhe técnico: ${error.message}`);
      return;
    }
  ...
  return (
    <>
      <Dialog open={salvamentoAberto} onOpenChange={setSalvamentoAberto}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Esta geração vira um item permanente — aparece na página Biblioteca e fica guardada no seu cofre.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <label htmlFor="salvar-titulo" className="mb-1.5 block text-xs font-medium text-muted-foreground">Título</label>
              <Input id="salvar-titulo" value={tituloSalvoF} onChange={(event) => setTituloSalvoF(event.target.value)} placeholder="Ex.: Hooks Verão Glow — prova social" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="salvar-categoria" className="mb-1.5 block text-xs font-medium text-muted-foreground">Categoria</label>
                <Select value={categoriaSalvaF} onValueChange={(valor) => setCategoriaSalvaF(valor as LibraryCategory)}>
                  <SelectTrigger id="salvar-categoria"><SelectValue placeholder="Selecione a categoria" /></SelectTrigger>
                  <SelectContent>
                    {categoriasBiblioteca.map((cat) => (
                      <SelectItem key={cat.valor} value={cat.valor}>{cat.rotulo}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div>
                <label htmlFor="salvar-autor" className="mb-1.5 block text-xs font-medium text-muted-foreground">Autor</label>
                <Input id="salvar-autor" value={autorSalvoF} onChange={(event) => setAutorSalvoF(event.target.value)} placeholder="Equipe AnuncIA" />
              </div>
            </div>
            {erroSalvamento && <p role="alert" className="text-sm text-red-400">{erroSalvamento}</p>}
          </div>
          <DialogFooter>
            <DialogClose asChild><Button type="button" variant="outline">Cancelar</Button></DialogClose>
            <Button variant="ai" onClick={() => void handleSalvarNaBiblioteca()} disabled={salvandoNaBiblioteca}>
              {salvandoNaBiblioteca ? <><Loader2 className="animate-spin" /> Gravando…</> : <><BookMarked /> Gravar na biblioteca</>}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      <PageHeader title="IA Studio" badge="Agentes exclusivos" description="Agentes especializados para cada etapa do processo.">
        <Button variant="ai" onClick={() => document.getElementById("studio-prompt")?.focus()}><Sparkles /> Nova Geração</Button>
      </PageHeader>
        ...
        {stats.map((stat) => (
          <Card key={stat.label} className="card-glow">
            <CardContent className="flex items-center gap-3 p-4">
              <div className={`flex size-9 shrink-0 items-center justify-center rounded-lg ${stat.tone}`}><stat.icon className="size-4" /></div>
      ...
      <h2 className="mt-8 text-sm font-semibold tracking-wide text-muted-foreground uppercase">Mesa de motores</h2>
      <p className="mt-1 mb-3 text-xs text-muted-foreground">A cadeia tenta nesta ordem e pula em silêncio quem não tem chave armada — você só fica sem resposta se TODOS falharem.</p>
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {provedores.map((provedor) => {
          const armado = armadoNaMesa(mesa, provedor.id);
          const conectado = armado === true;
          const rotuloSelo = mesa === undefined ? "Lendo…" : mesa === null ? "Sem leitura" : conectado ? "Conectado" : "Sem chave";
          return (
            <Card key={provedor.id} className={conectado ? "h-full border-success/40 shadow-[0_0_0_1px_rgba(16,185,129,0.25),0_0_24px_rgba(16,185,129,0.08)]" : "card-glow h-full opacity-70"}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex size-9 items-center justify-center rounded-lg bg-ai/15 text-ai"><provedor.icone className="size-4" /></div>
                  <Badge variant={conectado ? "success" : "secondary"}>{rotuloSelo}</Badge>
                </div>
                <p className="mt-3 text-sm font-semibold">{provedor.nome}</p>
                <p className="text-[11px] text-muted-foreground">{provedor.detalhe}</p>
                <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{provedor.descricao}</p>
              </CardContent>
            </Card>
          );
        })}
      ...
      <div className="mt-8 grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Settings2 className="size-5 text-primary" />Configuração da geração</CardTitle>
          ...
          <CardContent className="space-y-5">
            <div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {agents.map((agent) => {
                  ...
                  return (
                    <button key={agent.name} type="button" onClick={() => setSelectedAgent(agent.name)} aria-pressed={active} className={cn("flex cursor-pointer items-center gap-2.5 rounded-xl border p-2.5 text-left transition-all", active ? "border-ai/50 bg-ai/10" : "border-border hover:border-[rgba(255,255,255,0.16)]")}>
                      <div className={cn("flex size-8 shrink-0 items-center justify-center rounded-lg", active ? "bg-ai/20 text-ai" : "bg-muted text-muted-foreground")}><agent.icon className="size-4" /></div>
                      <div className="min-w-0">
                        <p className="truncate text-xs font-semibold">{agent.name}</p>
                        <p className="truncate text-[10px] text-muted-foreground">{agent.description}</p>
                      </div>
                    </button>
                  );
                })}
            ...
            <div className="rounded-xl border border-border p-3 text-xs leading-relaxed text-muted-foreground">
              <span className="font-medium text-foreground">Mesa em cadeia: </span>
              o app tenta o Gemini primeiro; se faltar chave ou a cota do dia acabar, cai para o próximo motor armado sem te pedir nada.
            </div>
            <div>
              <label htmlFor="studio-prompt" className="mb-2 block text-xs font-medium text-muted-foreground">Tarefa para o agente</label>
              <Textarea id="studio-prompt" value={promptText} onChange={(event) => setPromptText(event.target.value)} placeholder="Descreva o que o agente deve gerar... Ex.: Crie 5 hooks para a campanha Verão Glow focando em prova social." className="min-h-[120px]" />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-border p-3">
                <div className="mb-2 flex items-center justify-between text-xs"><span className="font-medium text-muted-foreground">Temperatura</span><span className="font-mono-params text-foreground">{temperature.toLocaleString("pt-BR")}</span></div>
                <input type="range" min={0} max={1} step={0.1} value={temperature} onChange={(event) => setTemperature(Number(event.target.value))} aria-label="Temperatura do modelo" className="w-full accent-primary" />
                <p className="mt-1 text-[10px] text-muted-foreground">0 = mais certeira · 1 = mais criativa</p>
              </div>
              <div className="rounded-xl border border-border p-3">
                <div className="mb-2 flex items-center justify-between text-xs"><span className="font-medium text-muted-foreground">Máx. tokens</span><span className="font-mono-params text-foreground">{maxTokens}</span></div>
                <input type="range" min={200} max={4000} step={200} value={maxTokens} onChange={(event) => setMaxTokens(Number(event.target.value))} aria-label="Máximo de tokens" className="w-full accent-ai" />
                <p className="mt-1 text-[10px] text-muted-foreground">roteiros longos = aumente pra não cortar no meio</p>
              </div>
            </div>
            <Button variant="ai" className="w-full" onClick={() => void handleGenerate()} disabled={generating || !promptText.trim()}>
              {generating ? <Loader2 className="animate-spin" /> : <Sparkles />}{generating ? "Gerando resposta..." : "Gerar resposta"}
            </Button>
          </CardContent>
        </Card>
        ...
        <Card className="flex flex-col">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sparkles className="size-5 text-ai" />Saída do agente</CardTitle>
            <CardDescription>Resultado real, pronto para copiar e colar</CardDescription>
          </CardHeader>
              ...
              {generating ? (
                <div className="space-y-3">
                  {[85, 95, 60, 90, 40].map((width) => (
                    <div key={width} style={{ width: `${width}%` }} className="h-3 animate-pulse rounded-full bg-muted" />
                  ))}
                  <p className="pt-2 text-xs text-muted-foreground">{selectedAgent} pensando…</p>
                </div>
              ) : erro ? (
                <div role="alert" className="rounded-lg border border-[rgba(239,68,68,0.35)] bg-[rgba(239,68,68,0.1)] px-3 py-2 text-sm text-red-300">{erro}</div>
              ) : output ? (
                <pre className="font-mono-params text-sm leading-relaxed whitespace-pre-wrap text-muted-foreground">{output}</pre>
              ) : (
                <div className="flex h-full min-h-[240px] flex-col items-center justify-center text-center">
                  <Sparkles className="size-8 text-muted-foreground/40" />
                  <p className="mt-3 text-sm font-medium">Nenhuma geração ainda</p>
                  <p className="mt-1 max-w-xs text-xs text-muted-foreground">Escolha um agente, descreva a tarefa e clique em Gerar resposta.</p>
                </div>
              )}
            </div>
            {output && !generating && (
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <p className="text-[11px] text-muted-foreground">{motorSaida ? `Respondido por ${motorSaida}` : ""}</p>
                <div className="flex flex-wrap gap-2">
                  <Button variant="outline" size="sm" onClick={() => copyText("output", output)}>{copiedTarget === "output" ? <Check className="text-success" /> : <Copy />}{copiedTarget === "output" ? "Copiado" : "Copiar"}</Button>
                  <Button variant="outline" size="sm" onClick={handleUsarNoGeradorImagem} disabled={gerandoImagem}><ImageIcon /> Usar no gerador de imagem</Button>
                  <Button variant="ai" size="sm" onClick={() => abrirSalvamento({ alvo: "saida-atual", agente: selectedAgent, prompt: promptText, output, motor: motorSaida ?? rotuloMotor(null) })}>{salvoRecente === "saida-atual" ? <><Check className="text-success" /> Salvo na biblioteca</> : <><BookMarked /> Salvar na biblioteca</>}</Button>
                </div>
              </div>
            )}
          </CardContent>
      ...
      <h2 className="mt-8 text-sm font-semibold tracking-wide text-muted-foreground uppercase">Gerador de imagem</h2>
      <p className="mt-1 mb-3 text-xs text-muted-foreground">Peça ao Engenheiro, clique em “Usar no gerador”, edite se quiser e gere. Referência opcional — já reduz pra 512.</p>
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><ImageIcon className="size-5 text-ai" />Prompt da imagem</CardTitle>
            <CardDescription>Em inglês elite agora — Mesa não precisa traduzir se já for elite</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Textarea id="gerador-imagem-prompt" value={imagemPromptF} onChange={(event) => setImagemPromptF(event.target.value)} placeholder="Cole aqui o prompt do Engenheiro (ou escreva o seu). Ex.: A single extra-large RED strawberry..." className="min-h-[120px]" />
            <div className="flex flex-wrap items-center gap-2">
              <label className="text-[11px] text-muted-foreground">Referência (opcional):</label>
              <input type="file" accept="image/*" onChange={handleReferenciaImagem} aria-label="Escolher imagem de referência" className="max-w-[220px] text-[11px] text-muted-foreground file:mr-2 file:cursor-pointer file:rounded-md file:border file:border-border file:bg-transparent file:px-2 file:py-1 file:text-[11px] file:text-muted-foreground" />
              {imagemReferencia && (
                <span className="flex items-center gap-2">
                  <img src={imagemReferencia} alt="Referência escolhida" className="size-10 rounded-md border border-border object-cover" />
                  <button type="button" onClick={() => setImagemReferencia(null)} className="cursor-pointer text-muted-foreground transition-colors hover:text-red-400">✕</button>
                </span>
              )}
            </div>
            <div className="flex flex-col gap-2 sm:flex-row">
              <Select value={imagemFormatoF} onValueChange={handleFormatoChange}>
                <SelectTrigger aria-label="Formato da imagem" className="w-full sm:max-w-[240px]"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <Button variant="ai" onClick={() => void handleGerarImagem()} disabled={gerandoImagem || !imagemPromptF.trim()} className="shrink-0">
                {gerandoImagem ? <><Loader2 className="h-4 w-4 animate-spin" /> Gerando…</> : <><ImageIcon /> Gerar imagem</>}
              </Button>
            </div>
            {erroImagem && <p role="alert" className="text-sm text-red-400">{erroImagem}</p>}
          </CardContent>
        </Card>

        <Card className="flex flex-col">
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Wand2 className="size-5 text-primary" />Prévia da imagem</CardTitle>
            <CardDescription>Nasce aqui — não estica mais, gera nova no formato</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-1 flex-col">
            <div className="flex flex-1 items-center justify-center rounded-xl border border-border bg-[rgba(255,255,255,0.02)] p-4 overflow-hidden">
              {gerandoImagem ? (
                <div className="flex flex-col items-center gap-3 py-10">
                  ...
                  <p className="text-xs text-muted-foreground">A Mesa de Imagens está pintando… (SDXL Lightning primeiro, melhor pra produto)</p>
                </div>
              ) : imagemGerada ? (
                <img src={imagemGerada} alt="Imagem gerada pela IA da AnuncIA" className="max-h-[420px] w-auto rounded-xl border border-border object-contain" />
              ) : (
                <div className="flex h-full min-h-[240px] flex-col items-center justify-center text-center">
                  <ImageIcon className="size-8 text-muted-foreground/40" />
                  <p className="mt-3 text-sm font-medium">Nenhuma imagem ainda</p>
                  <p className="mt-1 max-w-xs text-xs text-muted-foreground">Mude o formato e gere de novo — agora não estica, gera nova no formato certo.</p>
                </div>
              )}
            </div>
              ...
              <div className="mt-3 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-3">
                    <a href={imagemGerada} download="anuncia-criativo.png" className="text-xs text-muted-foreground underline underline-offset-2 hover:text-foreground">Baixar</a>
                    <Button variant="secondary" size="sm" onClick={() => void handleSalvarImagemEmMidias()} disabled={salvandoImagemMidia} className="h-7 text-xs">
                      {salvandoImagemMidia ? <><Loader2 className="mr-1 h-3 w-3 animate-spin" /> Salvando…</> : <>Salvar em Mídias + Biblioteca</>}
                    </Button>
                  </div>
                </div>
                {erroSalvarImagem && <p role="alert" className="text-xs text-red-400">{erroSalvarImagem}</p>}
                {sucessoSalvarImagem && <p role="status" className="text-xs text-emerald-400">{sucessoSalvarImagem}</p>}
                {promptImagemUsado && <p className="text-[11px] leading-relaxed text-muted-foreground">Prompt enviado: {promptImagemUsado}</p>}
                {imagemNotas && imagemNotas.length > 0 && (
                  <div>
                    <p className="text-[10px] font-semibold tracking-wide text-muted-foreground/70 uppercase">Diagnóstico</p>
                    <ul className="mt-0.5 space-y-0.5">
                      {imagemNotas.map((nota, indice) => (
                        <li key={`${indice}-${nota.slice(0, 12)}`} className="text-[10.5px] leading-relaxed text-muted-foreground/80">• {nota}</li>
                      ))}
                    </ul>
                  </div>
                )}
      ...
      <Card className="mt-8">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><History className="size-5 text-muted-foreground" />Histórico de gerações</CardTitle>
          <CardDescription>Suas gerações reais — vivem nesta sessão; salve os melhores na Biblioteca</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {historico.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Nenhuma geração ainda nesta sessão — a sua primeira está a um clique. ✨</div>
          ) : (
            historico.map((item) => (
              <div key={item.id} className="rounded-xl border border-border p-4 transition-colors hover:border-[rgba(255,255,255,0.16)]">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] text-muted-foreground">{item.hora}</span>
                    <Button variant="ghost" size="icon" onClick={() => abrirSalvamento({ alvo: item.id, agente: item.agente, prompt: item.prompt, output: item.output, motor: item.modelo })} className="size-8 text-muted-foreground">{salvoRecente === item.id ? <Check className="size-3.5 text-success" /> : <BookMarked className="size-3.5" />}</Button>
                    <Button variant="ghost" size="icon" onClick={() => copyText(item.id, item.output)} className="size-8 text-muted-foreground">{copiedTarget === item.id ? <Check className="size-3.5 text-success" /> : <Copy className="size-3.5" />}</Button>
                  </div>
                </div>
                <div className="mt-3 grid gap-3 lg:grid-cols-2">
                  <div className="rounded-lg border border-border bg-[rgba(255,255,255,0.02)] p-3"><p className="mb-1 text-[10px] font-semibold tracking-wider text-muted-foreground uppercase">Entrada</p><p className="text-xs leading-relaxed">{item.prompt}</p></div>
                  <div className="rounded-lg border border-ai/20 bg-ai/5 p-3"><p className="mb-1 text-[10px] font-semibold tracking-wider text-ai uppercase">Saída</p><p className="text-xs leading-relaxed text-muted-foreground">{item.output}</p></div>
                </div>
              </div>
            ))
          )}
