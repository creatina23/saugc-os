...
export function ClientesView() {
  ...
  const stats = [
    { label: "Total de clientes", value: clients.length.toString(), icon: Users, tone: "bg-primary/15 text-primary" },
    { label: "Clientes ativos", value: activeClients.toString(), icon: UserCheck, tone: "bg-success/15 text-success" },
    { label: "MRR total", value: formatBRL(totalMrr), icon: Wallet, tone: "bg-ai/15 text-ai" },
    {
      label: "Ticket médio",
      value: formatBRL(Math.round(totalMrr / clients.length)),
      icon: TrendingUp,
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Cadastre uma nova empresa na base da agência.
              </DialogDescription>
            </DialogHeader>
            <form
              className="space-y-4"
            ...
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <Input id="client-name" placeholder="Ex.: Mariana Costa" />
                </div>
                <div>
                  <label htmlFor="client-company" className={fieldLabel}>
                    Empresa
                  </label>
                  <Input id="client-company" placeholder="Ex.: Vitória Moda" />
                </div>
                <div>
                  <label htmlFor="client-email" className={fieldLabel}>
                    E-mail
                  </label>
                  <Input id="client-email" type="email" placeholder="contato@empresa.com.br" />
                </div>
                <div>
                  <label htmlFor="client-phone" className={fieldLabel}>
                    Telefone
                  </label>
                  <Input id="client-phone" placeholder="(11) 98765-4321" />
                </div>
                ...
                <TableRow key={client.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-1">
                      <Button variant="ghost" size="icon" aria-label={`Editar ${client.company}`} className="size-8 text-muted-foreground">
                        <Pencil className="size-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" aria-label={`Excluir ${client.company}`} className="size-8 text-muted-foreground hover:text-destructive">
